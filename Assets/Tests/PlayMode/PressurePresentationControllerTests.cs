...
namespace Daeume.Tests.PlayMode
{
    public sealed class PressurePresentationControllerTests
    {
        ...
        [UnityTest]
        public IEnumerator Test_Presentation_StableRemovesLastAppliedShakeOffset()
        {
            if (GameManager.Instance != null) Object.DestroyImmediate(GameManager.Instance.gameObject);
            var managerGo = new GameObject("Manager");
            managerGo.AddComponent<GameManager>();
            ...
            var cameraGo = new GameObject("Camera");
            var camera = cameraGo.AddComponent<Camera>();
            var basePosition = new Vector3(4f, 2f, -10f);
            ...
            try
            {
                GameManager.Instance.Events.Publish(
                    new ContaminationPressureChanged("test-variant", PressureStage.Intrusion, string.Empty));
                controller.SendMessage("Update");
                controller.SendMessage("LateUpdate");
                Assert.That(camera.transform.position, Is.Not.EqualTo(basePosition),
                    "Stable 복귀 전에 실제 흔들림 오프셋이 적용돼야 테스트 전제가 성립한다.");

                GameManager.Instance.Events.Publish(
                    new ContaminationPressureChanged("test-variant", PressureStage.Stable, string.Empty));

                Assert.That(Vector3.Distance(camera.transform.position, basePosition), Is.LessThan(0.00001f),
                    "Stable 이벤트 처리 즉시 마지막 흔들림 오프셋을 부동소수점 허용 오차 안에서 제거해야 한다.");
                yield return null;
            }
            finally
            {
