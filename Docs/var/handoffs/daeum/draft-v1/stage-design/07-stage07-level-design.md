...
- 제안값(실측 시 조정): 얼음 표면 마찰 계수 0.1~0.2(기본 지형 0.6~0.8 대비), 균열 발판은
  진입 후 1.2초 유예 뒤 파괴, 파괴돼도 낙하 복귀 계단 마커로 복귀 가능(즉사 없음 충족).
- 제안값: `VisualTraitTags` 인간형 암시 강도는 실루엣 윤곽 10~15% 수준(구체적 신체 부위
  노출 없음 — 100% 노출은 Stage11에서 최초). Role C 아트가 이 범위 내에서 확정.
