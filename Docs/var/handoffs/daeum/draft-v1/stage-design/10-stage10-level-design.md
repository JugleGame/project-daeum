...
- 제안값: Zone B 지형 3종 연속 배치 예상 통과 시간 25~35초 — director는 이 스테이지
  `TargetChaseSeconds` 하한을 40초 이상으로 잡아야 구간을 다 통과하기 전에 추격이 끝나지
  않는다(정확한 값은 director 튜닝 담당 확정).
- 제안값: "가까운 형태" = 카메라 기준 3~5m 근접 컷(Stage1~9는 원경 실루엣 위주였던 것과 대비).
